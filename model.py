...
def preprocess_text(text):
    ...
    # Hapus Tahun (angka 4 digit)
    text = re.sub(r'\b\d{4}\b', ' ', text)

    # Hapus DOI
    text = re.sub(r'\b10\.\d{4,9}/[-._;()/:a-z0-9]+\b', ' ', text)

    # Hapus ISSN / e-ISSN / p-ISSN
    text = re.sub(r'\b(e-issn|p-issn)[:\s]*\d{4}-\d{3}[\dxX]\b', ' ', text)

    # Hapus semua URL/link (termasuk www dan domain .com/.org/.id/.net/.ac)
    text = re.sub(r'\b(?:https?://|www\.)\S+\b', ' ', text)
    text = re.sub(r'\b\S+\.(com|id|org|net|ac|edu|co|gov)\b', ' ', text)

    # Hapus info volume, nomor, hal, licensi
    text = re.sub(r'\b(vol(?:ume)?|no(?:mor)?|hal(?:aman)?|licensed?|license|creativecommons|attribution|noncommercial|4\.0|international)\b[\s\S]{0,30}', ' ', text)
